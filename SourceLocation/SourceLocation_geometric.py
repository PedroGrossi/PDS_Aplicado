...
def emulate_source(xs_emulated, ys_emulated, s, Nt, Nx, Ny, a1, a2, c2, h, view=False):
    ...
    for nt in range(Nt):
        p_1, p_2 = p_0, p_1
        p_0 = a1 * p_1 + a2 * p_2 + c2 * ndi.convolve(p_1, h)
        p_0[ys_emulated, xs_emulated] += s[nt]
...
def emulate_source_single_sensor(xs, ys, Nt, Nx, Ny, a1, a2, c2, h, sensor_positions): # parei aqui
    p_0 = np.zeros((Ny, Nx), dtype=np.float32)
    p_1 = np.zeros((Ny, Nx), dtype=np.float32)
    p_2 = np.zeros((Ny, Nx), dtype=np.float32)
...
def cost_function(pos):
    x = int(np.clip(int(round(pos[0])), 1, Nx-2))
    y = int(np.clip(int(round(pos[1])), 1, Ny-2))
    
    sinais_sim = emulate_source_single_sensor(x, y, Nt, Nx, Ny, a1, a2, c2, h, sensor_positions)
...
plt.plot(xs, ys, 'ro', label='Original Source')
plt.plot(xs_est, ys_est, 'bx', label='Estimated Source')
plt.legend()
